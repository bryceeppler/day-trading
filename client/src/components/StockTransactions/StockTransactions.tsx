...
import { StockTransaction, WalletTransaction } from 'types/users.types';
              ...
              <div className={styles.company}>
                ...
                <div className={styles.value}>{item.order_status}</div>
							...
							{["IN_PROGRESS", "PARTIAL_FULFILLED"].includes(item.order_status) && (
								<div className={styles.company}>
									<Button label='Cancel' style={BUTTON_TYPE.WHITE} onClick={() => cancelTransaction(item.stock_tx_id)}/>
								</div>
							)}
